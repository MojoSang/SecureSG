...
public class EDBSetup_keyUtils {
     ...
     * 双线性对
     */
    private static final Pairing pairing = PairingFactory.getPairing("com/zhong/params/curves/a.properties");
    ...
    private static void SerializeKey() {
        File file = MyUtils.getFile("com/zhong/output_keys", "allKeys.keys");
        if (file.exists()) {
    ...
    private static void SerializeFreshKey() {
        File file = MyUtils.getFile("src/main/java/com/zhong/output_keys", "allKeys.keys");
        MK mk = new MK(Kx, KI, Kz, Ks, MyUtils.generateK_T(), new SerializableElement(g));
        SerializationDemonstrator.serialize(mk, file.getAbsolutePath());
    }
    ...
    public static MK getFreshKey(){
        File file = MyUtils.getFile("output_keys", "allKeys.keys");
        MK mk = EDBSetup_keyUtils.DeserializeKey();
        MK mkn=new MK(mk.getKx(),mk.getKI(),mk.getKz(),mk.getKs(),MyUtils.generateK_T(),new SerializableElement(mk.getG()));
        SerializationDemonstrator.serialize(mkn, file.getAbsolutePath());
        return EDBSetup_keyUtils.DeserializeKey();
    }
