...
public class SearchProtocol_Split_memory {
    ...
    public static Search_Client_1_Output search_client_1(ArrayList<String> kws) throws Exception {
        ...
        // 计算token
        Element xtoken[][] = new Element[800][kws.size()];
        //预测：最多有c个文件包含关键词w1
    ...
    public static Search_Client_1_Output search_client_1BS(ArrayList<String> kws) throws Exception {
        mk = EDBSetup_keyUtils.getKey();
        pairing = PairingFactory.getPairing("com/zhong/params/curves/a.properties");
        //群的生成元
        Element g = mk.getG();
        ...
        return new Search_Client_1_Output(kws.get(0),xtoken);
    }
     ...
     * @return 搜索的结果，是密文形式的
     */
    public static ArrayList<byte[]> search_server(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        //搜索的结果
    ...
    public static ArrayList<t_item> search_server_Single(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {



        //搜索的结果
        ArrayList<t_item> t = T_Sets_split_memory.TSetRetrieve(search_client_1_output.stag,table_name);
...
//            System.out.println("t null");
    ...
    public static ArrayList<t_item> search_server_SingleBS(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {


        //搜索的结果
        ArrayList<t_item> t = T_Sets_split_memory.TSetRetrieveBS(search_client_1_output.getW(),table_name);
        //经过匹配筛选后的结果
        ArrayList<t_item> es=new ArrayList<t_item>();
//        if (t != null) {
//            System.out.println("tsize:"+t.size());
...
//            }
    ...
    public static HashMap<byte[],ArrayList<byte[]>> search_server_Multi(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        System.out.println("retrieve finish:"+(t2-t1)+"ms");
        return res;
    }

    public static HashMap<String,ArrayList<byte[]>> search_server_MultiBS(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {
        ...
        //返回结果存放
        //ind <w1,weig1>,<w2,weig>
        HashMap<String,ArrayList<byte[]>> res=new HashMap<>();
    ...
    //返回整个t队列，用于拿取权值和文件名
    public static ArrayList<t_item> search_server_v2(Search_Client_1_Output search_client_1_output, String table_name) throws UnsupportedEncodingException, NoSuchAlgorithmException {
...
//                x_item xt=(x_item) mt.byte2Msg(xx);
    ...
    public static HashMap<String,ArrayList<x_item>> search_client_2_MultiBS(String w1,HashMap<String,ArrayList<byte[]>> es) throws IOException, NoSuchAlgorithmException, NoSuchProviderException, InvalidKeyException, InvalidAlgorithmParameterException, NoSuchPaddingException {


        //返回结果
        HashMap<String,ArrayList<x_item>> res=new HashMap<>();

        for (Map.Entry<String, ArrayList<byte[]>> arrayListEntry : es.entrySet()) {
            //解密ind
            String ind=arrayListEntry.getKey();
            ArrayList<x_item> xs=new ArrayList<>();
            for (byte[] bytes : arrayListEntry.getValue()) {
                x_item x=(x_item) MyUtils.byte2Msg(bytes);
                xs.add(x);
